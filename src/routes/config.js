...
router.post('/', async (req, res) => {
  ...
  const { rotationInterval, tickerRefreshInterval, maxTickerItems, tickerEnabled, refreshEnabled, refreshInterval, primaryTeamId, secondaryTeamIds } =
    req.body;
  ...
  if (refreshEnabled !== undefined) {
    const v = typeof refreshEnabled === 'string' ? refreshEnabled.trim().toLowerCase() : refreshEnabled;
    if (v === true || v === 'true' || v === 1 || v === '1') {
      validated.refreshEnabled = true;
    } else if (v === false || v === 'false' || v === 0 || v === '0') {
      validated.refreshEnabled = false;
    } else {
      errors.push(
        'refreshEnabled must be a boolean, the string "true"/"false", or numeric 1/0',
      );
    }
  }

  if (refreshInterval !== undefined) {
    const v = Number(refreshInterval);
    if (!Number.isInteger(v) || v <= 0) {
      errors.push('refreshInterval must be a positive integer (milliseconds)');
    } else {
      validated.refreshInterval = v;
    }
  }
